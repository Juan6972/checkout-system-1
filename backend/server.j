...
app.post('/api/orders', paymentLimiter, async (req, res) => {
  ...
  try {
    ...
    console.log('[FuriaPay] Resposta completa:', JSON.stringify(data, null, 2));
...
// A API FuriaPay usa snake_case nas respostas
  ...
  if (method === 'pix') {
    return {
      ...
      pix: {
        ...
        qrCodeBase64: data.pix?.qr_code_url || data.pix?.qr_code_base64,
